<x-layouts.app>
    <div class="max-w-6xl mx-auto px-4 py-6">
        {{-- Cover Photo --}}
        @if($user->cover_photo)
            <div class="h-64 rounded-t-lg overflow-hidden mb-4">
                <img src="{{ $user->cover_photo }}" alt="Cover" class="w-full h-full object-cover">
            </div>
        @else
            <div class="h-64 rounded-t-lg bg-gradient-to-r from-indigo-500 to-purple-600 mb-4"></div>
        @endif

        {{-- Profile Header --}}
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 -mt-20 relative mb-6">
            <div class="flex flex-col md:flex-row items-center md:items-end">
                ...
                <div class="md:ml-6 mt-4 md:mt-0 text-center md:text-left flex-1">
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100 flex items-center justify-center md:justify-start">
                        {{ $user->name }}
                        @if($user->hasVerifiedBadge())
                            <svg class="w-7 h-7 ml-2 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                        @elseif($user->hasPurpleBadge())
                            <svg class="w-7 h-7 ml-2 text-purple-500" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                        @endif
                    </h1>
                    ...
                    <div class="flex items-center justify-center md:justify-start space-x-6 mt-4">
                        <div class="text-center">
                            <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $user->posts()->count() }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Posts</p>
                        </div>
                        <a href="{{ route('profile.followers', $user) }}" class="text-center hover:text-indigo-600 dark:hover:text-indigo-400 transition">
                            <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $user->followers()->count() }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Followers</p>
                        </a>
                        <a href="{{ route('profile.following', $user) }}" class="text-center hover:text-indigo-600 dark:hover:text-indigo-400 transition">
                            <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $user->following()->count() }}</p>
                    ...
                    <div class="flex flex-col md:flex-row gap-3 mt-4 md:mt-0">
                        <form action="{{ auth()->user()->isFollowing($user) ? route('follow.destroy', $user) : route('follow.store', $user) }}"
                            ...
                            @if(auth()->user()->isFollowing($user))
                                @method('DELETE')
                                <button type="submit" class="px-6 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition font-semibold">
                                    Following
                                </button>
                            @else
                        ...
                        <a href="{{ route('messages.show', $user) }}" class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold text-center">
                            Message
                        </a>
                    </div>
                @endif
        ...
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">
                {{ $user->id === auth()->id() ? 'Your Posts' : $user->name . "'s Posts" }}
            </h2>
            ...
            @empty
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-12 text-center">
                    ...
                    <p class="mt-2 text-gray-500 dark:text-gray-400">This user hasn't posted anything yet.</p>
                </div>
            @endforelse
